...
local DEBUG = {
    ...
    ERRORS = false,
    UI = false,
    LOGS = false,	-- disable logs on release
}
...
local function ProcessPvPVictory()
    ...
    if pvpType == "Arena" then
        local myTeamIndex = GetBattlefieldArenaFaction() -- 0 lub 1
        ...
        playerTeam = (UnitFactionGroup("player") or "Unknown")
        local w = GetBattlefieldWinner()
        if w == 1 then
            winner = "Alliance"
...
ArenaTracker:SetScript("OnEvent", function(_, event, arg1)
    ...
    if not ArenaTeams.isActive then
        print("Arena detected! Initializing tracker...")
    ...
    if event == "ARENA_OPPONENT_UPDATE" then
        if not ArenaTeams.isInitialized then
            C_Timer.After(1, function()
                ...
                if ok then
                    print("Arena teams loaded after opponent update.")
                end
            end)
        end
        return
